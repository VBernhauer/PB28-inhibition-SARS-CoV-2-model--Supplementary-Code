...
def ode_kinetics(state, t, parameters, eps):
    ...
    dT = - (1 - eps) * parameters[0] * T * V
    dE[0] = (1 - eps) * parameters[0] * T * V - N_E / parameters[1] * E[0]
    for i in range(1, len(E), 1):
        dE[i] = N_E / parameters[1] * (E[i - 1] - E[i])
    dI = N_E / parameters[1] * E[N_E - 1] - 1 / parameters[2] * I
    dV = parameters[3] * I - omega * V
...
for ii in range(3):
    for jj in range(3):
        ...
        if kk <= 5:
            axs[ii, jj].fill_between(tt, solution_min_96well[kk], solution_max_96well[kk], facecolor="lightgreen",
                                                                                           alpha=alpha)
        ...
        if kk > 5:
            ...
            for line_data in data_virus[mm]:
                axs[ii,jj].plot(t_file_virus, np.log10(line_data), marker="o",
                                                                       ...
                                                                       alpha=alpha_data)
fig.tight_layout()
plt.savefig("./figures/FigS14.tiff", format="tiff")
plt.savefig("./figures/FigS14.pdf", format="pdf", transparent=True)
plt.show()